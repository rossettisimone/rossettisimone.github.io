...
function populateUseCases() {
  ...
  const userFilters = [...new Set(useCases.flatMap((uc) => (uc["User Segment"] ? uc["User Segment"].split(", ") : [])))];
  const categoryFilters = [...new Set(useCases.map((uc) => (uc.Category ? uc.Category.replace(/"/g, "").split(", ")[0] : "Other")))];
    ...
    personaModalContent.innerHTML = `
            ...
            <div class="mt-6">
                ...
                <div class="flex flex-wrap gap-2">
                    ${persona.preferredFeatures
                      .map((feature) => `<span class="bg-brand-offwhite px-3 py-1 rounded-full text-sm text-brand-charcoal">${feature}</span>`)
                      .join("")}
                </div>
            </div>
        `;
