...
class LegalPagesI18n {
  ...
  updateCookiePolicySections() {
    ...
    h3Elements.forEach((h3) => {
      ...
      if (text.includes("a)") || text.includes("Strictly Necessary")) {
        ...
        h3.textContent = this.get(
          "legalPages.cookiePolicy.sections.typesOfCookies.performanceAnalytics.title",
    ...
    const legalBasisSection =
      this.findSectionByTitle("3. Base Legale per il Trattamento") || this.findSectionByTitle("3. Legal Basis for Processing");
  ...
  updatePrivacyPolicyYourRights() {
    ...
    if (yourRightsSection) {
      ...
      while (currentElement && currentElement.tagName !== "H2") {
        if (
          (currentElement.tagName === "P" && currentElement.textContent.includes("To exercise your rights")) ||
          currentElement.textContent.includes("Per esercitare i tuoi diritti")
        ) {
